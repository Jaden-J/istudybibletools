    ...
    public abstract partial class BaseSupplementalForm : Form
    {
        ...
        private void CommitChanges(bool closeAfter)
        {
            ...
            catch (Exception ex)
            {
                ...
                MainForm.LongProcessingDone(string.Format("{0}: {1}", BibleCommon.Resources.Constants.ErrorOccurred, OneNoteUtils.ParseErrorAndMakeItMoreUserFriendly(ex.Message)));
            }                        
        ...
        protected List<string> TryToUseExistingNotebook(string notebookId, string notebookName)
        {
            ...
            var pagesEls = xDoc.Root.XPathSelectElements(string.Format("//one:Page[{0}]", OneNoteUtils.NotInRecycleXPathCondition), xnm);  здесь надо проверить
